...
const ProjectDetails = () => {
    ...
    useEffect(() => {
        fetch(base + "/Tehran/Project?id=" + id ).then((response) =>
    ...
    return (
        <div className={classes.content}>
            <div className='Details'>
                <div className='Card'>
                    <img src={`http://charity.mykanoon.ir/File/Get/${data?.project?.imageIds[0]}`} alt="ProjectPic" className='Pic'/>
                </div>
                <div className='project-info'>
                    <h1 className='Title'>{data?.project?.title}</h1>
                    <h3>وقف شده توسط : </h3>
                    <Link to={`/Philanthropists/${data?.project?.philanthropist?.id}`} className='flex-div'>
                        <Button variant='contained' className='button'>
                            <div>
                                <img src={`http://charity.mykanoon.ir/File/Get/${data?.project?.philanthropist?.imageId}`} alt="PhiloPic" className='Philo-pic'/>
                            </div>
                            <h2 className='Philo-name'>
                                {`${data?.project?.philanthropist?.firstName} ${data?.project?.philanthropist?.lastName ? data?.project?.philanthropist?.lastName : ''}`}
                            </h2>
                        </Button>
                    </Link>
                    <div className='Details-div'>
                        <div className='Flex-div'>
                            <div>
                                <div>تاریخ انجام پروژه : {startDate} - {endDate}</div>
                                <div>محل احداث : {data?.project?.cityName}</div>
                            </div>
                            <div>
                                <div>بودجه : {ToFarsiNumber(NumberCreator(data?.project?.fund))} تومان</div>
